...
export default function UBSectionsPage() {
  const [sections, setSections] = useState<UBSection[]>([]);
  ...
  const columns: ColumnDef<UBSection>[] = [
    ...
    },
    {
      ...
      header: createSortableHeader('Mass (kg/m)'),
    },
    {
      accessorKey: 'surface_area_per_unit_length_m2m',
      header: createSortableHeader('Surface Area (m²/m)'),
    },
    {
      accessorKey: 'volume_per_unit_length_cm3m',
      header: createSortableHeader('Volume (cm³/m)'),
    },
    {
      accessorKey: 'surface_area_to_volume_ratio_1m',
      header: createSortableHeader('Surface Area/Volume (1/m)'),
    },
    {
  ...
  useEffect(() => {
    const fetchSections = async () => {
      try {
        const response = await fetch('/api/ub-sections');
          ...
          <TableHeader>
            {table.getHeaderGroups().map((headerGroup) => (
              <TableRow key={headerGroup.id}>
                {headerGroup.headers.map((header: Header<UBSection, unknown>) => (
