...
import { ucSections } from '../schema/schema.uc';
...
export async function seedUCSections() {
  try {
    // Read the CSV file
    const csvFilePath = join(process.cwd(), 'src/app/databases/uc-sections.csv');
    console.log('Reading UC sections from:', csvFilePath);
    ...
    const existingSections = await db.select({ section: ucSections.section }).from(ucSections);
    console.log(`Found ${existingSections.length} existing sections in database`);
    console.log('Existing sections:', existingSections.map(s => s.section).join(', '));
    
    const existingSectionNames = new Set(existingSections.map(s => s.section));

    // Transform the data to match the database schema
    const sections = records
      .filter(record => !existingSectionNames.has(record.Section))
      .map((record) => {
        if (!record.Section) {
    ...
    console.log(`Transformed ${sections.length} sections (skipped ${records.length - sections.length} existing sections)`);
    if (sections.length > 0) {
      console.log('New sections to be inserted:', sections.map(s => s.section).join(', '));
    }

    if (sections.length > 0) {
      // Insert the data into the database
      console.log('Inserting UC sections data...');
      await db.insert(ucSections).values(sections);
      console.log('Successfully inserted UC sections data!');
    } else {
      console.log('No new UC sections to insert.');
    }
  } catch (error) {
    console.error('Error seeding UC sections:', error);
    throw error;
  }
} 