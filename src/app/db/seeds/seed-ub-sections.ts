...
export async function seedUBSections() {
  try {
    // Read the CSV file
    ...
    const existingSections = await db.select({ section: ubSections.section }).from(ubSections);
    ...
    if (sections.length > 0) {
      ...
      console.log('No new UB sections to insert.');
    }
  } catch (error) {
    console.error('Error seeding UB sections:', error);
    throw error;
  }
} 